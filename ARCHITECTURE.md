...
two jobs they are actually good at: reading a photo, and putting a result into words.
...
(`npm test` — 22 assertions today).

## Why the agents run in the background

A person asking "should I replace this PC?" does not want a chat session. They want to
hand over one photo and get an answer. So the work fans out the moment the image lands,
and the user watches the fleet report in rather than answering follow-up questions.

The expensive part — finding out what the market *would* have sold them — is the slowest
leg and the one nobody can do by hand. It runs asynchronously alongside everything else.

## The fleet

```
                       photo / screenshot            "I just browse and use Excel"
                              │                                  │
                              ▼                                  ▼
                   ┌──────────────────┐              ┌──────────────────────┐
                   │  SCANNER agent   │              │   WORKLOAD agent     │
                   │  (Gemini vision) │              │   (Gemini text)      │
                   │  image → parts   │              │   words → profiles   │
                   └────────┬─────────┘              └──────────┬───────────┘
                            │ raw part names                    │ workload ids
                            ▼                                   │
                   ┌──────────────────┐                         │
                   │  RESOLVER fleet  │  ← N lookups in parallel│
                   │  name → canonical│                         │
                   │  + benchmark data│                         │
                   └────────┬─────────┘                         │
                            │                                   │
              ...
              ▼                             ▼
   ┌─────────────────────┐      ┌──────────────────────────┐
   │  COUNTER-OFFER      │      │   VERDICT ENGINE         │
   │  agent (async)      │      │   ★ no LLM ★             │
   │  what the market    │      │   core/verdict.js        │
   │  would sell you     │      │   deterministic, tested  │
   └──────────┬──────────┘      └────────────┬─────────────┘
              │  market price                │  keep / fix / overkill
              └──────────────┬───────────────┘
                             ▼
                   ┌──────────────────┐
                   │  NARRATOR agent  │
...
|---|---|---|---|
| **Scanner** | Gemini (vision) | Read a photo of the inside of a PC, a Device Manager screenshot, or a spec sheet, and name the components | Asking for the one field it could not read, instead of guessing |
...
| **Counter-offer** | Gemini + search | Find what a typical retailer recommends for this workload, so the gap can be shown | Omitting the comparison rather than inventing a price |
...
| **Narrator** | Gemini (text) | Explain the verdict in the user's language | Never introducing a number that the engine did not produce |
...
core/          the judgment engine — no network, no model, fully testable
  ...
  selftest.js    22 assertions over the logic above
